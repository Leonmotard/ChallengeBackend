...
public interface MovieRepository extends JpaRepository<Movie, Long> {
	...
	public List<Movie> findByGenero(Genre genero);
	
	@Transactional
	@Modifying
	@Query(value= "UPDATE movie m SET m.titulo=?2, m.fecha_de_creacion = ?3, m.calificacion = ?4, m.genero_id= ?5  WHERE m.id= ?1  ", nativeQuery= true)
